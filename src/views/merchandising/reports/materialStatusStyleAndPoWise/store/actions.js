...
//Get Data by Query
export const fetchMaterialStatusStyleAndPoItemDetails = ( styleId, itemCategoryId, index ) => async dispatch => {
  const response = await baseAxios.get(
    `${MATERIAL_STATUS_STYLE_AND_PO_WISE_API.fetch_material_status_style_po_item_details( styleId, itemCategoryId )}`
  );
  const data = response.data.data;
  dispatch( {
