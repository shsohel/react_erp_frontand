/*
  Title: WeeklyShipmentSchedule
  Description: WeeklyShipmentSchedule
...
import { fetchWeeklyShipmentSchedule } from '../store/actions';

const WeeklyShipmentSchedule = () => {
  //#region hooks
  const history = useHistory();
  const dispatch = useDispatch();
  const { items } = useSelector( ( { weeklyShipmentScheduleReducer } ) => weeklyShipmentScheduleReducer );
  // console.log(items);
  //#endregion

  //#region Effects
  useEffect( () => {
    dispatch( fetchWeeklyShipmentSchedule() );
  }, [dispatch] );
  //#endregion
  ...
  return (
    <div className="p-1 mt-3">
      <ActionMenu title="Weekly Shipment Schedule" moreButton={false} breadcrumb={breadcrumb}>
      ...
      <Card>
        <Row className="pt-2 pr-3 pl-3 pb-1">
          <FormGroup tag={Col} xs={12} sm={12} md={12} lg={12} xl={12} className="mt-n1 pt-1">
            <Badge color="primary">{`Weekly Shipment Schedule`}</Badge>
          </FormGroup>
          ...
          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
            <Table responsive bordered hover size="sm" className="custom-table">
              <thead className={`text-center table-bordered`}>
                <tr>
                  <th colSpan={2}>Pending Schedule</th>
...
export default WeeklyShipmentSchedule;
