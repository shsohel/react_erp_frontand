...
import { fetchMasterOrderSummaryByBuyerPO } from '../store/actions';
import { FROM_DATE_CHANGE_ORDER_SUMMARY_BY_BUYER_PO, LOADING } from '../store/actionType';

const MasterOrderSummaryByBuyerPO = () => {
  //#region hooks
  // const history = useHistory();
  const dispatch = useDispatch();
  const { orderSummaryByBuyerPo, loading } = useSelector( ( { masterOrderSummaryByBuyerPOReducer } ) => masterOrderSummaryByBuyerPOReducer );
  // console.log(orderSummaryByBuyerPo);
  //#endregion
  ...
  //#region Effects
  useEffect( () => {
    // dispatch(fetchMasterOrderSummaryByBuyerPO());
  }, [dispatch] );
  //#endregion
  ...
  const onFromDateChange = date => {
    setFromDate( date );
    dispatch( { type: FROM_DATE_CHANGE_ORDER_SUMMARY_BY_BUYER_PO } );
  };
  ...
  const onReportView = () => {
    ...
    dispatch( fetchMasterOrderSummaryByBuyerPO( serverDate( fromDate ), lastDateFromMonth( toDates ) ) );
  ...
  const totalQuantity = months => {
    const sum = orderSummaryByBuyerPo?.orderSummaryList
  ...
  return (
    <div className="p-1 mt-3">
      <ActionMenu title="Master Order Summary By Buyer PO" moreButton={false} breadcrumb={breadcrumb}>
      ...
      {loading && fromDate ? (
        <div>
          {orderSummaryByBuyerPo?.headings ? (
            <div>
                    ...
                    <Table responsive bordered hover size="sm" className="custom-table">
                      <thead className={`text-center table-bordered`}>
                        <tr>
                          ...
                          {orderSummaryByBuyerPo?.headings?.map( osl => (
                      ...
                      <tbody className={`text-right`}>
                        {orderSummaryByBuyerPo?.orderSummaryList?.map( osl => (
                          <Fragment key={uuid()}>
                            <tr>
                            ...
                            {osl.styleBuyerList?.map( sbl => (
                              <tr key={uuid()}>
                                <td className={`text-left`}>{sbl.orderNumber}</td>
                            ...
                            <tr className="font-weight-bold">
                              ...
                              {orderSummaryByBuyerPo?.headings?.map( mol => (
                                <Fragment key={uuid()}>
                  ...
                  <Col xs={12}>
                    <Row className="d-flex justify-content-between">
                      <div>
                        ...
                        <span>{` ${orderSummaryByBuyerPo?.reportGeneratedOn}`}</span>
                      </div>
                      <div>
                        <Label className="h5">User: </Label>
                        <span>{` ${orderSummaryByBuyerPo?.user}`}</span>
                      </div>
                    </Row>
                  </Col>
                </Row>
...
export default MasterOrderSummaryByBuyerPO;
