/*
  Title: MaterialRequirementByStyleDetails
  Description: MaterialRequirementByStyleDetails
...
import { fetchMaterialRequirementByStyleDetails } from '../store/actions';

const MaterialRequirementByStyleDetails = () => {
  //#region hooks
  const history = useHistory();
  const dispatch = useDispatch();
  const { items } = useSelector( ( { materialRequirementByStyleDetailsReducer } ) => materialRequirementByStyleDetailsReducer );
  // console.log(items);
  //#endregion

  //#region Effects
  useEffect( () => {
    dispatch( fetchMaterialRequirementByStyleDetails() );
  }, [dispatch] );
  //#endregion
  ...
  return (
    <div className="p-1 mt-3">
      <ActionMenu title="Material Requirement By Style Details" moreButton={false} breadcrumb={breadcrumb}>
      ...
      <Card className="custom-report-form">
        <Row className="pt-2 pr-3 pl-3 pb-1">
          <FormGroup tag={Col} xs={12} sm={12} md={12} lg={12} xl={12} className="mt-n1 pt-1">
            <Badge color="primary">{`Material Requirement By Style Details`}</Badge>
          </FormGroup>
...
export default MaterialRequirementByStyleDetails;
