/*
  Title: MaterialRequirementItemDetailsPoWise
  Description: MaterialRequirementItemDetailsPoWise
...
import { fetchMaterialRequirementItemDetailsPoWise } from '../store/actions';

const MaterialRequirementItemDetailsPoWise = () => {
  //#region hooks
  const history = useHistory();
  const dispatch = useDispatch();
  const { items } = useSelector( ( { materialRequirementItemDetailsPoWiseReducer } ) => materialRequirementItemDetailsPoWiseReducer );
  // console.log(items);
  //#endregion

  //#region Effects
  useEffect( () => {
    dispatch( fetchMaterialRequirementItemDetailsPoWise() );
  }, [dispatch] );
  //#endregion
  ...
  return (
    <div className="p-1 mt-3">
      <ActionMenu title="Material Requirement Item Details Po Wise" moreButton={false} breadcrumb={breadcrumb}>
      ...
      <Card className="custom-report-form">
        <Row className="pt-2 pr-3 pl-3 pb-1">
          <FormGroup tag={Col} xs={12} sm={12} md={12} lg={12} xl={12} className="mt-n1 pt-1">
            <Badge color="primary">{`Material Requirement Item Details Po Wise`}</Badge>
          </FormGroup>
...
export default MaterialRequirementItemDetailsPoWise;
