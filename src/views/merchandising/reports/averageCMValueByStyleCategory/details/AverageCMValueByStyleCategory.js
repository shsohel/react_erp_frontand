/*
  Title: AverageCMValueByStyleCategory
  Description: AverageCMValueByStyleCategory
...
import { fetchAverageCMValueByStyleCategory } from '../store/actions';

const AverageCMValueByStyleCategory = () => {
  //#region hooks
  const history = useHistory();
  const dispatch = useDispatch();
  const { items } = useSelector( ( { averageCMValueByStyleCategoryReducer } ) => averageCMValueByStyleCategoryReducer );
  // console.log(items);
  //#endregion

  //#region Effects
  useEffect( () => {
    dispatch( fetchAverageCMValueByStyleCategory() );
  }, [dispatch] );
  //#endregion
      ...
      <Card>
        <Row className="pt-2 pr-3 pl-3 pb-1">
          <FormGroup tag={Col} xs={12} sm={12} md={12} lg={12} xl={12} className="mt-n1 pt-1">
            <Badge color="primary">{`Six Month Order Value By Style ( jul, 2022 -Dec, 2022 )`}</Badge>
          </FormGroup>

          <Col xs={12} sm={12} md={12} lg={12} xl={12}>
            <CanvasJSChart options={items} />
...
export default AverageCMValueByStyleCategory;
