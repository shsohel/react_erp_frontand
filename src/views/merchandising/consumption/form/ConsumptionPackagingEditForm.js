...
import { bindConsumptionBasicInfo, cleanAllConsumptionState, deletePackConsumptionPurchaseOrderDetails, getConsumptionPackOrderDetails, getPackConsumptionById, updatePackConsumption } from '../store/actions';
...
const ConsumptionPackagingEditForm = () => {
    ...
    const handleDeleteConsumptionPODetails = ( id, rowId ) => {
        if ( id ) {
            dispatch( deletePackConsumptionPurchaseOrderDetails( consumptionId, id, rowId ) );
        } else {
            confirmDialog( confirmObj ).then( async e => {
                if ( e.isConfirmed ) {
                    const updatedData = consumptionBasicInfo.packOrderDetails.filter( o => o.rowId !== rowId );
    ...
    const onSubmit = () => {
        ...
        dispatch( updatePackConsumption( submitObj, push, consumptionId ) );
    };
    const handleCancel = () => {
    ...
    return (
        <div>
            <Card className="mt-3">
                <CardBody>
                    <ActionMenu breadcrumb={breadcrumb} title='Edit Consumption Packaging' >
...
export default ConsumptionPackagingEditForm;