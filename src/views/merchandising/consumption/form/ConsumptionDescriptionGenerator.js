...
import { bindConsumptionFabricDetails } from '../store/actions';


const ConsumptionDescriptionGenerator = ( { descriptionModalObj, openModal, setOpenModal } ) => {
    ...
    const handleModalSubmit = () => {
        ...
        const isValueArrayExiting = consumptionFabricDetails.map( fd => fd.itemDescriptionArray.some( ida => ida.value === createDescriptionWithValue.trim() ) ).some( isTrue => isTrue === true );
        const isValueExiting = consumptionFabricDetails.some( fd => fd.itemDescription === createDescriptionWithValue.trim() );
        const updatedData = consumptionFabricDetails.map( i => {
            if ( descriptionModalObj.consumptionFieldId === i.fieldId ) {
    ...
    return (
        <div>
            <CustomModal
                ...
                title="Item Description For Fabric"
            >
                <Row className=' mt-1'>
...
export default ConsumptionDescriptionGenerator;
