...
import { deleteRangeStyleCategory, getStyleCategoryByQuery, handleOpenStyleCategorySidebar, handleOpenStyleCategorySidebarForEdit } from '../store/actions';
import StyleCategoryExpandRow from './StyleCategoryExpandRow';
import { handleGetStyleCategoryColumns } from './StyleCategoryTableColumn';
...
const StyleCategoryList = () => {
    ...
    const {
        ...
        openStyleCategorySidebar,
        openStyleCategorySidebarForEdit,
        queryData,
        isStyleCategoryDataLoaded
    } = useSelector( ( { styleCategories } ) => styleCategories );
    ...
    const toggleSidebarForEdit = () => store.dispatch( handleOpenStyleCategorySidebarForEdit( !openStyleCategorySidebarForEdit ) );
    ...
    const getAllStyleCategories = () => {
        dispatch(
            getStyleCategoryByQuery( paramsObj, filteredData )
        );
    };
    ...
    const handleClearFilterBox = () => {
        ...
        dispatch( getStyleCategoryByQuery( paramsObj, [] ) );
    };

    const handleSearch = () => {
        setCurrentPage( 1 );
        dispatch(
            getStyleCategoryByQuery( { ...paramsObj, page: 1 }, filteredData )
        );
    };
        ...
        <div>
            <Card className="mt-3">
                <ActionMenu
                    ...
                    moreButton={isPermit( userPermission?.StyleCategoryCreate, authPermissions )}
                    middleNavButton={
                ...
                >
                    <NavItem
                        className="mr-1"
                        hidden={!isPermit( userPermission?.StyleCategoryCreate, authPermissions )}
                    >
                        <NavLink
                            tag={Button}
                            size="sm"
                ...
                <CardBody>
                    <div >
                        ...
                        <DataTable
                            ...
                            sortServer
                            progressPending={!isStyleCategoryDataLoaded}
                            progressComponent={
                                <CustomPreLoader />
                            }
                            dense
                            ...
                            expandOnRowClicked
                            persistTableHead
                            columns={handleGetStyleCategoryColumns( userPermission, authPermissions )}
            ...
            {
                ( selectedStyleCategory !== null && openStyleCategorySidebarForEdit ) && < StyleCategoryEditForm data={selectedStyleCategory} open={openStyleCategorySidebarForEdit} toggleSidebarForEdit={toggleSidebarForEdit} />
            }
            {
                openStyleCategorySidebar && <StyleCategoryAddForm open={openStyleCategorySidebar} toggleSidebar={toggleSidebar} />
            }
