...
import { deleteRangeDivision, getDepartmentsByDivisionId, getDivisionByQuery, handleOpenDivisionSidebar, handleOpenDivisionSidebarForEdit } from '../store/actions';
import DivisionExpandRow from './DivisionExpandRow';
import { handleGetDivisionColumns } from './DivisionTableColumns';
...
const DivisionList = () => {
    ...
    const {
        ...
        selectedDivision,
        openDivisionSidebar,
        openDivisionSidebarForEdit,
        queryData,
        isDivisionDataLoaded
    } = useSelector( ( { divisions } ) => divisions );
    ...
    const toggleSidebarForEdit = () => store.dispatch( handleOpenDivisionSidebarForEdit( !openDivisionSidebarForEdit ) );
    ...
    const getAllDivisions = () => {
        dispatch( getDivisionByQuery( paramsObj, filteredData )
        );
    };
    useEffect( () => {
        getAllDivisions();
    ...
    const handleClearFilterBox = () => {
        ...
        dispatch( getDivisionByQuery( paramsObj, [] ) );
    };

    const handleSearch = () => {
        setCurrentPage( 1 );
        dispatch(
            getDivisionByQuery( { ...paramsObj, page: 1 }, filteredData )
        );
    };
        ...
        <div>
            <Card className="mt-3">
                <ActionMenu
                    ...
                    title='Style Divisions'
                    moreButton={isPermit( userPermission?.StyleDivisionCreate, authPermissions )}
                    middleNavButton={
                ...
                >
                    <NavItem
                        className="mr-1"
                        hidden={!isPermit( userPermission?.StyleDivisionCreate, authPermissions )}
                    >
                        <NavLink
                            tag={Button}
                            size="sm"
                ...
                <CardBody>
                    <div >
                        ...
                        <DataTable
                            ...
                            sortServer
                            progressPending={!isDivisionDataLoaded}
                            progressComponent={
                                <CustomPreLoader />
                            }
                            ...
                            responsive={true}
                            paginationServer
                            onRowExpandToggled={( expanded, row ) => dispatch( getDepartmentsByDivisionId( row.id ) )}

                            expandableRows
                            expandableRowsComponent={<DivisionExpandRow data={data => data} />}
                            expandOnRowClicked
                            persistTableHead
                            columns={handleGetDivisionColumns( userPermission, authPermissions )}
            ...
            {
                openDivisionSidebarForEdit && < DivisionEditForm data={selectedDivision} open={openDivisionSidebarForEdit} toggleSidebarForEdit={toggleSidebarForEdit} />
            }
            {
                openDivisionSidebar && <DivisionAddForm open={openDivisionSidebar} toggleSidebar={toggleSidebar} />
            }
        </div>
    );
};
