...
import { Button, ButtonGroup, Card, CardBody, CardHeader, CardTitle, Col, CustomInput, Form, FormFeedback, FormGroup, Input, Label, Row, Table } from 'reactstrap';
...
const SetStyleAddFormTest = () => {
    ...
    console.log( isSetStyleFileUploadComplete );
    ...
    useEffect( () => {
        ...
        if ( lastSetStyleId ) {
            dispatch( getSetStyleById( lastSetStyleId ) );
        ...
        const styleDetails = styleCombination.map( sd => (
            {
                ...
                sizeId: sd.size?.value,
                colorId: sd.color?.value,
                quantity: sd.quantity
            } ) );

        if ( !colorSizeSpecification.sizeSpecific ) {
            styleDetails.map( sd => { delete sd.sizeId; } );
        }
        if ( !colorSizeSpecification.colorSpecific ) {
            styleDetails.map( sd => { delete sd.colorId; } );
        }
        const obj = {
...
export default SetStyleAddFormTest;
