...
import { bindProcurementBasicInfo, bindProcurementItems, getProcurementById, updateProcurement } from '../store/actions';
...
const ProcurementEditForm = () => {
  ...
  const onSubmit = () => {
    ...
    if ( submitObj.details.length ) {
      ...
      if ( !isDeferent ) {
        dispatch( updateProcurement( submitObj, procurementId, '', procurementBasicInfo?.updateStatus?.label, push ) );
      } else {
        notify( 'warning', 'Same item should be same Order UOM!!!' );
      }
    } else {
  ...
  return <div>
    <ActionMenu breadcrumb={breadcrumb} title='Edit IPO' >
...
export default ProcurementEditForm;
