...
import { Copy, Edit3, MinusSquare, PlusSquare, Trash2, XSquare } from 'react-feather';
...
import { isHaveDeferentObj, isZeroToFixed, randomIdGenerator, selectThemeColors } from '../../../../utility/Utils';
...
const NormalProcurementItemDetails = () => {
    ...
    console.log( dropDownItemGroups );
                            ...
                            },
                            {
                                ...
                                cell: row => <span>
                                    <Button.Ripple id="deleteFabId"
                                        ...
                                        onClick={() => { handleDuplicateItemRow( row.rowId ); }}
                                        className='btn-icon p-0 '
                                        color='flat-success'
                                    >
                                        <Copy size={18} id="deleteFabId" color="green" />
                                    </Button.Ripple>
                                    <Button.Ripple id="deleteFabId"
                                        tag={Label}
                                        // disabled={( orderDetails.length === 1 )}
                                        ...
                                        color='flat-success'
                                    >
                                        <Edit3 size={18} id="deleteFabId" color={row.isEditable ? 'grey' : 'green'} />
                                    </Button.Ripple>
                                    <Button.Ripple id="deleteFabId"
                                        tag={Label}
                                        // disabled={( orderDetails.length === 1 )}
                                        onClick={() => { handleItemDelete( row.rowId ); }}
                                        className='btn-icon p-0 ml-1'
                                        color='flat-danger'
                                    >
                            ...
                            },
                            {
                                ...
                                cell: row => (
                                    row.isEditable ? <span className='w-100'> <Select
                                        ...
                                        isSearchable
                                        isCreatable={false}
                                        //  isDisabled={stylePurchaserOrder?.style?.isSizeSpecific}
                                        maxMenuHeight={200}
                                        menuPosition="fixed"
                                        theme={selectThemeColors}
                                        options={buyerStylesDropdown}
                                        classNamePrefix="dropdown"
                                        className={classnames( `erp-dropdown-select ${( ( row.isFieldError && !row.style ) ) && 'is-invalid'}` )}
                                        value={row.style}
                                        onFocus={() => { handleStyleDropdownOnFocus(); }}
                                        onChange={( data, e ) => {
                                            handleControlDropdownChange( data, e, row.rowId );
                                        }}

                                    /> </span> : row?.style?.label
                                )
                            },
                            {
                                ...
                                cell: row => (
                                    row.isEditable ? <span className='w-100'> <Select
                                        ...
                                        menuPosition="fixed"
                                        theme={selectThemeColors}
                                        options={dropDownItemGroupByGroups}
                                        classNamePrefix="dropdown"
                                        className={classnames( `erp-dropdown-select ${( ( row.isFieldError && !row.item ) ) && 'is-invalid'}` )}
                                        ...
                                        onFocus={() => {
                                            handleItemGroupDropdownOnFocus( procurementBasicInfo?.itemGroupType?.value );
                                        }}
                                    /> </span> : row?.itemGroup?.label
                                )
                            },
                            {
                                ...
                                cell: row => (
                                    row.isEditable ? <span className='w-100'> <Select
                                        ...
                                        onFocus={() => {
                                            handleItemSubGroupDropdownOnFocus( row.itemGroup?.value, row.rowId, row.itemSubGroups );
                                        }}
                                    /> </span> : row?.itemSubGroup?.label
                                )
                            ...
                            },
                            {
                                ...
                                cell: row => (
                                    row.isEditable ? <span className='w-100'> <Select
                                        ...
                                        onFocus={() => {
                                            handleItemDescriptionOnFocus( row.itemGroup?.value, row.itemSubGroup?.value, row.rowId );
                                        }}
                                    /> </span> : row?.item?.label
                                )
                            },
...

                            {
                                ...
                                cell: row => (
                                    row.isEditable ? <span className='w-100'> <Select
                                        ...
                                        menuPosition="fixed"
                                        theme={selectThemeColors}
                                        options={row.orderUoms}
                                        classNamePrefix="dropdown"
                                        className={classnames( `erp-dropdown-select ${( ( row.isFieldError && !row.orderUom ) ) && 'is-invalid'}` )}
                                        value={row.orderUom}
                                        onChange={( data, e ) => {
                                            handleControlDropdownChange( data, e, row.rowId );
                                        }}
                                        onFocus={() => {
                                            handleOrderUomDropdownOnFocus(
                                                dropDownItemGroupByGroups?.find( item => item.value === row.itemGroup?.value )?.defaultUomSetId, row.orderUoms, row.rowId
                                            );
                                        }}
                            ...
                            },
                            {
                                ...
                                cell: row => (
                                    row.isEditable ? <NumberFormat
                                        className={classnames( `form-control-sm form-control text-right ${( ( row.isFieldError && row.orderQuantity === 0 ) ) && 'border-danger'}` )}
                                        displayType="input"
                                        value={row.orderQuantity}
                                        name="orderQuantity"
                                        decimalScale={4}
                                        allowNegative={false}
                                        fixedDecimalScale={row.orderQuantity !== 0}
                                    ...
                                    /> : isZeroToFixed( row.orderQuantity, 4 )
                                )
                            },
                            {
                                id: 'orderRate',
                                ...
                                cell: row => (
                                    row.isEditable ? <NumberFormat
                                        className={classnames( `form-control-sm form-control text-right ${( ( row.isFieldError && row.orderRate === 0 ) ) && 'border-danger'}` )}
                                        displayType="input"
                                        value={row.orderRate}
                                        name="orderRate"
                                        decimalScale={4}
                                        allowNegative={false}
                                        fixedDecimalScale={row.orderRate !== 0}
                                        allowLeadingZeros={false}
                                        onFocus={e => {
                                            e.target.select();
                                        }}
