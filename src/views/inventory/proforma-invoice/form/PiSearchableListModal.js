...
import { bindPiBasicInfo, bindProcurementItems, bindSelectedProcurementItems, bindSelectedSupplierOrders, bindSupplierOrderOnChange, getPiByQuery } from '../store/actions';
...
const PiSearchableListModal = ( { openModal, setOpenModal, searchFor } ) => {
    ...
    const { total, queryData, isPiDataLoaded } = useSelector( ( { pis } ) => pis );
    ...
    const handleGetPIData = ( piId ) => {
        if ( searchFor === "pi-details" ) {
            push( { pathname: `/pi-details`, state: `${piId.id}` } );

        } else {
            push( { pathname: `/edit-pi`, state: `${piId.id}` } );
        }
        setOpenModal( !openModal );
                ...
                <Card >
                    ...
                    <CardBody>
                        ...
                        <div >
                            ...
                            <DataTable
                                noHeader
                                onRowDoubleClicked={( row ) => { handleGetPIData( row ); }}
...
export default PiSearchableListModal;
