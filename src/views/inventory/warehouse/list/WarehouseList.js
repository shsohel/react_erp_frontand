import AdvancedSearchBox from '@custom/AdvancedSearchBox';
...
import { bindWarehouseDataOnchange, getWarehousesByQuery } from '../store/actions';
import { handleGetWarehouseColumns } from './WarehouseTableColumns';
...
const WarehouseList = () => {
    ...
    const { isWarehouseDataLoaded, queryData, total } = useSelector( ( { warehouses } ) => warehouses );
    ...
    const getAllWarehouses = () => {
        dispatch(
            getWarehousesByQuery( paramsObj, filteredData )
        );
    };
    ...
    const handleClearFilterBox = () => {
        ...
        dispatch( getWarehousesByQuery( paramsObj, [] ) );
    };

    const handleSearch = () => {
        setCurrentPage( 1 );
        dispatch(
            getWarehousesByQuery( { ...paramsObj, page: 1 }, filteredData )
        );
    };
        ...
        <div>
            <Card className="mt-3">
                <ActionMenu
                    ...
                    title='Warehouses'
                    moreButton={isPermit( userPermission?.WarehouseCreate, authPermissions )}
                ...
                >
                    <NavItem
                        className="mr-1"
                        hidden={!isPermit( userPermission?.WarehouseCreate, authPermissions )}
                    >
                        <NavLink
                            tag={Button}
                            size="sm"
                ...
                <CardBody>
                    <div >
                        ...
                        <DataTable
                            ...
                            onSort={handleSort}
                            // contextMessage={}
                            progressPending={!isWarehouseDataLoaded}
                            progressComponent={
                                <CustomPreLoader />
                            }
                            ...
                            paginationServer
                            expandOnRowClicked
                            columns={handleGetWarehouseColumns( userPermission, authPermissions )}
